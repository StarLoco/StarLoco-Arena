/*
 * Decompiled with CFR 0.152.
 */
import java.io.Reader;
import java.util.LinkedList;

public final class Xg
extends aqk_0
implements gx_2 {
    private static final String ayT = "lines";
    private static final String ayU = "skip";
    private static final int ayW = 10;
    private long ayX = 10L;
    private long ayY = 0L;
    private boolean bXa = false;
    private ia_0 ayZ = null;
    private String DV = null;
    private int anr = 0;
    private LinkedList bXb = new LinkedList();

    public Xg() {
    }

    public Xg(Reader reader) {
        super(reader);
        this.ayZ = new ia_0();
        this.ayZ.x(true);
    }

    public int read() {
        if (!this.aCg()) {
            this.initialize();
            this.bk(true);
        }
        while (this.DV == null || this.DV.length() == 0) {
            this.DV = this.ayZ.a(this.in);
            this.DV = this.gG(this.DV);
            if (this.DV == null) {
                return -1;
            }
            this.anr = 0;
        }
        char c = this.DV.charAt(this.anr);
        ++this.anr;
        if (this.anr == this.DV.length()) {
            this.DV = null;
        }
        return c;
    }

    public void aU(long l2) {
        this.ayX = l2;
    }

    private long En() {
        return this.ayX;
    }

    public void aV(long l2) {
        this.ayY = l2;
    }

    private long Eo() {
        return this.ayY;
    }

    public Reader b(Reader reader) {
        Xg xg = new Xg(reader);
        xg.aU(this.En());
        xg.aV(this.Eo());
        xg.bk(true);
        return xg;
    }

    private void initialize() {
        vj_0[] vj_0Array = this.JT();
        if (vj_0Array != null) {
            for (int j = 0; j < vj_0Array.length; ++j) {
                if (ayT.equals(vj_0Array[j].getName())) {
                    this.aU(Long.parseLong(vj_0Array[j].getValue()));
                    continue;
                }
                if (!ayU.equals(vj_0Array[j].getName())) continue;
                this.ayY = Long.parseLong(vj_0Array[j].getValue());
            }
        }
    }

    private String gG(String string) {
        if (!this.bXa) {
            if (string != null) {
                this.bXb.add(string);
                if (this.ayX == -1L) {
                    if ((long)this.bXb.size() > this.ayY) {
                        return (String)this.bXb.removeFirst();
                    }
                } else {
                    long l2 = this.ayX + (this.ayY > 0L ? this.ayY : 0L);
                    if (l2 < (long)this.bXb.size()) {
                        this.bXb.removeFirst();
                    }
                }
                return "";
            }
            this.bXa = true;
            if (this.ayY > 0L) {
                int n2 = 0;
                while ((long)n2 < this.ayY) {
                    this.bXb.removeLast();
                    ++n2;
                }
            }
            if (this.ayX > -1L) {
                while ((long)this.bXb.size() > this.ayX) {
                    this.bXb.removeFirst();
                }
            }
        }
        if (this.bXb.size() > 0) {
            return (String)this.bXb.removeFirst();
        }
        return null;
    }
}

