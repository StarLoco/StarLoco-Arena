/*
 * Decompiled with CFR 0.152.
 */
/*
 * Renamed from ih
 */
public class ih_2
extends ang_2 {
    byte xP;
    byte xQ;

    public void b(acf acf2) {
        super.b(acf2);
        this.xP = acf2.readByte();
        this.xQ = acf2.readByte();
    }

    public final float lt() {
        return (float)this.xP * 16.0f / 127.0f;
    }

    public final float lu() {
        return (float)this.xQ * 16.0f / 127.0f;
    }

    public final boolean lv() {
        return true;
    }

    public void a(pq_0 pq_02, pq_0 pq_03) {
        pq_03.acD = false;
        if (pq_02.acD) {
            pq_03.acx = (float)this.cJP / 127.0f;
            pq_03.acy = (float)this.cJQ / 127.0f;
            pq_03.acz = (float)this.cJR / 127.0f;
            pq_03.acA = (float)this.cJS / 127.0f;
        } else {
            pq_03.acx = (float)this.cJP / 127.0f * pq_02.acx + (float)this.cJQ / 127.0f * pq_02.acz;
            pq_03.acy = (float)this.cJP / 127.0f * pq_02.acy + (float)this.cJQ / 127.0f * pq_02.acA;
            pq_03.acz = (float)this.cJR / 127.0f * pq_02.acx + (float)this.cJS / 127.0f * pq_02.acz;
            pq_03.acA = (float)this.cJR / 127.0f * pq_02.acy + (float)this.cJS / 127.0f * pq_02.acA;
        }
        pq_03.acE = false;
        if (pq_02.acD) {
            pq_03.acB = (float)this.xP * 16.0f / 127.0f + pq_02.acB;
            pq_03.acC = (float)this.xQ * 16.0f / 127.0f + pq_02.acC;
        } else {
            pq_03.acB = (float)this.xP * 16.0f / 127.0f * pq_02.acx + (float)this.xQ * 16.0f / 127.0f * pq_02.acz + pq_02.acB;
            pq_03.acC = (float)this.xP * 16.0f / 127.0f * pq_02.acy + (float)this.xQ * 16.0f / 127.0f * pq_02.acA + pq_02.acC;
        }
        pq_03.IQ = pq_02.IQ;
        pq_03.IR = pq_02.IR;
        pq_03.IS = pq_02.IS;
        pq_03.IT = pq_02.IT;
    }
}

