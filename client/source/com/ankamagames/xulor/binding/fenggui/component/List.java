...
/*      */ 
/*      */ public class List extends ObservableWidget implements IBasicContainer, RenderableCollection {
/*   37 */   private ScrollBar m_scrollBar = null;
/*      */   
/*      */   private final List THIS;
/*      */   
/*   41 */   private Dimension m_wishedMinSize = null;
/*      */ 
/*      */   
...
/*      */   
/*   48 */   private Dimension m_cellSize = new Dimension();
/*      */ 
/*      */ 
/*      */ 
/*      */ 
...
/*      */   
/*      */   private boolean m_beingLayouted = false;
/*      */   
...
/*      */   
/*   96 */   private final ArrayList<IListSelectionChangedListener> m_selectionChangedListeners = new ArrayList<IListSelectionChangedListener>();
/*      */   
/*   98 */   private final ArrayList<ItemOutListener> m_itemOutListeners = new ArrayList<ItemOutListener>();
/*      */   
/*  100 */   private final ArrayList<ItemOverListener> m_itemOverListeners = new ArrayList<ItemOverListener>();
/*      */   
/*      */   public List() {
/*  103 */     this(false);
/*      */   }
...
/*  107 */     this.m_appearance = new ListAppearance((IWidget)this);
/*      */     
...
/*  125 */             List.this
/*  126 */               .setListOffset(List.this.sliderValueToOffset(sliderMovedEvent.getPosition()));
/*      */           }
/*      */         });
/*  129 */     this.m_renderables = new ArrayList<RenderableContainer>();
/*      */     
/*  131 */     this.THIS = this;
/*      */   }
/*      */ 
...
/*  189 */                   mouseWheelEvent.getLocalX((IWidget)List.this.THIS), 
/*  190 */                   mouseWheelEvent.getLocalY((IWidget)List.this.THIS))) {
...
/*  303 */       int i = getPotentialColumnCount(this.m_currentRowCount) - 
...
/*  315 */       int i = getPotentialColumnCount(this.m_currentRowCount) - 
/*  316 */         this.m_currentColumnCount + this.m_showOneMore;
/*  317 */       return (int)Math.round(i * value);
/*      */     } 
...
/*  416 */       RenderableContainer renderable = this.m_renderables.get(i);
...
/*      */         } 
/*  423 */         renderable.setItem(this.m_items.get(i + offset), true, updateAtOnce);
/*      */       } else {
/*  425 */         renderable.setItem((Item)null, true, updateAtOnce);
/*      */       } 
/*      */       
...
/*      */ 
/*      */   
/*      */   public void setWishedMinSize(Dimension wishedMinSize) {
...
/*      */ 
/*      */   
/*      */   public Dimension getIdealSize(int maxRows, int maxColumns) {
/*  565 */     Dimension dim = getContentIdealSize(maxRows, maxColumns);
...
/*      */   }
/*      */   
/*      */   public Dimension getContentIdealSize(int maxRows, int maxColumns) {
/*  576 */     int width = 10, height = 10;
/*      */ 
/*      */     
...
/*      */     }
/*      */     
/*  611 */     return new Dimension(width, height);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  638 */       Item item = this.m_items.get(i);
...
/*  647 */         fireSelectionChanged(new ListSelectionChangedEvent(this, getSelectedRenderableBySelectedOffset(this.m_selectedOffset), this.m_items.get(this.m_selectedOffset), false));
/*      */       }
/*  649 */       if (this.m_selectedOffset != -1) {
/*  650 */         fireSelectionChanged(new ListSelectionChangedEvent(this, getSelectedRenderableBySelectedOffset(this.m_selectedOffset), this.m_items.get(this.m_selectedOffset), true));
/*      */       }
...
/*  663 */       return null;
/*      */     }
/*  665 */     return this.m_items.get(this.m_selectedOffset);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*  678 */     int offset = 0;
/*  679 */     for (; offset < this.m_items.size() && (
/*  680 */       (Item)this.m_items.get(offset)).getValue() != value; offset++);
/*      */ 
/*      */ 
/*      */     
...
/*      */   }
/*      */   
/*      */   public RenderableContainer getSelectedRenderableBySelectedOffset(int offset) {
/*      */     int number;
...
/*  698 */       return null;
/*      */     }
/*  700 */     return this.m_renderables.get(number);
/*      */   }
/*      */ 
/*      */   
...
/*      */     } else {
/*  760 */       this.m_selectedValue = this.m_items.get(this.m_selectedOffset);
/*      */     } 
/*      */     
/*  763 */     if (old != null) {
...
/*      */     }
/*  826 */     else if (!this.m_alignment.equals(Alignment.CENTER)) {
/*  827 */       columnCount = (int)Math.floor(availableWidth / cellWidth);
/*      */     }
/*  829 */     else if (this.m_items != null && this.m_items.size() > 0) {
...
/*  869 */           this.m_displayScrollbar = true; break;
/*      */         } 
...
/*  950 */           container = (RenderableContainer)xcontainer.getWidget();
...
/*      */           }
/*      */           
/*  962 */           container.addMousePressedListener(new IMousePressedListener() {
...
/*      */                 }
/*      */               });
/*  975 */           container.addMouseExitedListener(new IMouseExitedListener() {
...
/*      */           }
/*      */         } else {
/*  986 */           container = this.m_renderables.get(offset);
/*      */         } 
/*  988 */         if (this.m_items != null && offset < this.m_items.size()) {
/*  989 */           container.setItem(this.m_items.get(offset));
/*      */         } else {
/*  991 */           container.setItem((Item)null);
/*      */         } 
/*      */         
...
/* 1007 */       ArrayList<RenderableContainer> toRemove = new ArrayList<RenderableContainer>(); int k;
/* 1008 */       for (k = 0; k < rowCount; k++) {
/* 1009 */         for (int l = j; l < this.m_currentColumnCount; l++) {
/* 1010 */           if (k * this.m_currentColumnCount + l < this.m_renderables.size()) {
/* 1011 */             toRemove.add(this.m_renderables
/* 1012 */                 .get(k * this.m_currentColumnCount + l));
/*      */           }
/*      */         } 
/*      */       } 
/* 1016 */       for (k = i; k < this.m_currentRowCount; k++) {
/* 1017 */         for (int l = 0; l < columnCount; l++) {
/* 1018 */           if (k * columnCount + l < this.m_renderables.size()) {
/* 1019 */             toRemove.add(this.m_renderables.get(k * columnCount + l));
/*      */           }
/*      */         } 
/*      */       } 
/*      */       
...
/* 1048 */           potentialColumnCount - this.m_currentColumnCount + 
/* 1049 */           this.m_showOneMore > 0.0D) {
...
/* 1057 */               potentialColumnCount + this.m_showOneMore));
/*      */         }
/*      */         else {
/*      */           
...
/* 1066 */           potentialRowCount - this.m_currentRowCount + 
/* 1067 */           this.m_showOneMore > 0.0D) {
...
/* 1075 */               potentialRowCount + this.m_showOneMore));
/*      */         }
/*      */         else {
/*      */           
...
/*      */       } 
/* 1119 */       return new Dimension(minWidth, minHeight);
/*      */     }
/*      */ 
/*      */ 
/*      */ 
...
/* 1152 */         IWidget c = List.this.m_renderables.get(i);
/*      */         
...
/* 1182 */     return this.m_items.<Item>toArray(new Item[0]);
/*      */   }
/*      */ 
/*      */ 
/*      */ 
...
/*      */     
/* 1191 */     this.m_items = new ArrayList<Item>();
/*      */     
/* 1193 */     if (items != null) {
/* 1194 */       byte b; int j; Item[] arrayOfItem; for (j = (arrayOfItem = items).length, b = 0; b < j; ) { Item item = arrayOfItem[b];
...
/* 1202 */       ((RenderableContainer)this.m_renderables.get(i)).setRenderer((ItemRenderer)null);
/*      */     }
/*      */     
...
/* 1344 */       this.m_items = new ArrayList<Item>();
/*      */     }
...
/* 1361 */       this.m_items = new ArrayList<Item>();
/*      */     }
...
/* 1409 */       int i; for (i = 0; i < this.m_items.size(); i++) {
...
/* 1434 */       return this.m_items.get(i);
/*      */     }
/* 1436 */     return null;
/*      */   }
/*      */ 
...
/* Location:              E:\Jeux\Ankama\DofusArena2-offi\game\core.jar!\com\ankamagames\xulor\binding\fenggui\component\List.class
