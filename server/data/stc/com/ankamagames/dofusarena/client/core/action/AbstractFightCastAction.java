...
/*     */     
/*     */ 
/*  57 */     addJavaFunctionsLibrary(MobileFunctionsLibrary.getInstance());
/*  58 */     addJavaFunctionsLibrary(ParticleSystemFunctionsLibrary.getInstance());
/*  59 */     addJavaFunctionsLibrary(SoundFunctionsLibrary.getInstance());
/*  60 */     addJavaFunctionsLibrary(VisualEffectFunctionsLibrary.getInstance());
/*  61 */     addJavaFunctionsLibrary(new FightCastActionFunctionsLibrary(this));
/*     */     
...
/*     */       
/*  88 */       PathMobile caster = ((Fighter)DofusArenaGameEntity.getInstance().getFight().getFighterById(getInstigatorId())).getActor();
/*     */       
/*  90 */       double cameraX = (caster.getWorldX() + getX()) / 2.0D;
/*  91 */       double cameraY = (caster.getWorldY() + getY()) / 2.0D;
/*  92 */       double cameraZ = (caster.getAltitude() + getZ()) / 2.0D;
/*     */       
/*  94 */       DefaultIsoWorldTarget actionCameraTarget = new DefaultIsoWorldTarget(cameraX, cameraY, cameraZ);
/*     */       
/*  96 */       camera.setTrackingTarget(actionCameraTarget);
...
/* Location:              C:\Users\flore\Desktop\DofusArena2-offi\game\core.jar!\com\ankamagames\dofusarena\client\core\action\AbstractFightCastAction.class
