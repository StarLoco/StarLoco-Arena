...
| **Rebound / renvoi de dommages** | ✅ | in-formula `damage.go:87-100` (`DmgRebound`); reactive `combat_actions.go:190-198` (`StrikeBackPercent`+`DmgRebound`) | 🔬 **To check:** two code paths both read `DmgRebound` — verify no double-count depending on entry point |
...
- 🔬 Verify rebound is not double-applied across its two paths (still open — a verification task, not a code gap).
...
**To do / verify:**
- 🔬 Re-validate A* against a **height-blocked / corner-cut** real-map path (deferred, roadmap Phase K).
...
| Line of sight | 🟡 | `line_of_sight.go` — Bresenham **approximation** of the reference 3D DDA (documented, not bit-exact) |
...
**To improve:**
- 🟡 LOS is a documented approximation — fine unless a specific spell needs bit-exact sub-cell traversal.
...
**Partial / to check:**
- 🟡 `RecvFighterUpdateInventoryRequest` (6011) — spell-blob parsed with a possibly-wrong per-entry size (flagged wire mismatch, `opcodes/00-implementation-status.md`).
...
10. ✅ ~~A* against height-blocked real-map path~~ — verified correct + tested (§11 V3).
...
| **V3** | A* height-blocking (Phase K) | **Verified correct.** `FindPath`/`ValidateClientPath` already respected `ArrivalAltitude` `blocked=true` and the `maxAscend`/`maxDescend` limits — but the altitude branches were untested (mocks always returned `blocked=false`). Added a height-aware provider + a **real-map** (`fightMapID=2`) integration test. | `TestFindPath_*` (height), `TestFindPath_RealMap*` |
