...
// decodeTGA decodes a Targa image into an image.NRGBA. It supports the
// variants that actually appear in this client's gfx.jar (uncompressed
// true-color, type 2, 32bpp BGRA -- confirmed by sampling real headers) plus
// the common near neighbours (RLE true-color type 10, and 24bpp) so the
// viewer is robust to any other sprite that shows up. Colour-mapped (type
// 1/9) and grayscale (type 3/11) are not needed for these assets and return
...
func decodeTGA(data []byte) (image.Image, error) {
	...
	if imageType == 2 {
		...
		// RLE (type 10): packets of a 1-byte header + pixel(s). Header
		// high bit set => run-length packet (repeat next pixel N+1 times);
		// clear => raw packet (N+1 literal pixels).
...
// decodeTGARLE expands an RLE (image type 10) pixel stream into dst (a flat
// bytesPerPixel-per-pixel buffer sized for the full image).
